...
public class Voxels {
    ...
    static final char THRESHOLD = 128;
        ...
        for(int x = 0; x < chunkResolution; x++) {
            for(int z = 0; z < chunkResolution; z++) {
                for(int y = 0; y < chunkHeight; y++) {
                    ...
                    if(volumeMap.data[y][x][z] < THRESHOLD)     // skip air cells
                        continue;
                    ...
                    if(y == chunkHeight-1 || volumeMap.data[y+1][x][z] < THRESHOLD)
                        makeTop(fx, fy, fz);
                    if(y == 0 || volumeMap.data[y-1][x][z]< THRESHOLD)
                        makeBottom(fx, fy, fz);
                    if(z == chunkResolution -1 || volumeMap.data[y][x][z+1] < THRESHOLD)
                        makeFront(fx, fy, fz);
                    if(z == 0 || volumeMap.data[y][x][z-1]< THRESHOLD)
                        makeBack(fx, fy, fz);
                    if(x == chunkResolution -1 || volumeMap.data[y][x+1][z]< THRESHOLD)
                        makeRight(fx, fy, fz);
                    if(x == 0 || volumeMap.data[y][x-1][z]< THRESHOLD)
                        makeLeft(fx, fy, fz);
                }
            }
        }
